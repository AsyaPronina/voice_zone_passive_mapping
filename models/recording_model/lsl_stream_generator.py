...
class LSL_Generator():
    def __init__(self, stream_time, channel_count, nominal_srate, q_from_display_to_recorder):
    ...
    def _stream_debug(self):
        ...
        outlet = pylsl.stream_outlet(info)
        print('DebudStream: Streaming start...')
        path_rest = 'C:/Workspace/PatientData/Sysoeva/10_10_19/data_rest/experiment_data.h5'
        path_actions = 'C:/Workspace/PatientData//Sysoeva/10_10_19/data_actions/experiment_data.h5'
        path_objects = 'C:/Workspace/PatientData/Sysoeva/10_10_19/data_objects/experiment_data.h5'
        paths = [path_rest, path_rest, path_actions, path_objects]
        self.q_from_display_to_recorder.put(('inlet_state', 1))
        time.sleep(3)
        for i in range(len(paths)):
            self.q_from_display_to_recorder.put(('patient_state',i-1))
            with h5py.File(paths[i], "r") as file:
                ...
                while (time.time() - start_time < self.stream_time) and index < 100000:
                    #print(index)
                    if time.time() - current_time > 1/4096:
                        current_time = time.time()
        ...
        with h5py.File(paths[0], "r") as file:
            ...
            start_time = time.time()
            current_time = time.time()
            while (time.time() - start_time < 5) and index < 100000:
                if time.time() - current_time > 1/4096:
                    current_time = time.time()
                    sample = data[index, :69]
                    outlet.push_sample(sample)
